...
//Helper functions for Game.cpp Draw related to Mesh
void Mesh::Draw()
{
...
{
	//Create vertex buffer and fill description
	{
		D3D11_BUFFER_DESC vbDescription = {};
		vbDescription.Usage = D3D11_USAGE_IMMUTABLE;
		vbDescription.ByteWidth = sizeof(Vertex) * numVertices;
		...
		device->CreateBuffer(&vbDescription, &initialVertexData, vBuffer.GetAddressOf());
	}
	//Create index buffer and fill description
	{
		D3D11_BUFFER_DESC ibDescription = {};
		ibDescription.Usage = D3D11_USAGE_IMMUTABLE;
		ibDescription.ByteWidth = sizeof(unsigned int) * numIndices;
		...
		device->CreateBuffer(&ibDescription, &initialIndexData, iBuffer.GetAddressOf());
	}
}


Mesh::Mesh(Vertex* vertexArray, int numVertices, unsigned int* indexArray, int numIndices,
	Microsoft::WRL::ComPtr<ID3D11Device> device, Microsoft::WRL::ComPtr<ID3D11DeviceContext> deviceContext) : indexCount(numIndices), deviceContext(deviceContext)
{
	//Create vertex index buffers
	CreateBuffers(vertexArray, numVertices, indexArray, numIndices, device);
}

Mesh::Mesh(const wchar_t* name, Microsoft::WRL::ComPtr<ID3D11Device> device, Microsoft::WRL::ComPtr<ID3D11DeviceContext> deviceContext) : indexCount(0), deviceContext(deviceContext)
{
...
// Purpose: Basic .OBJ 3D model loading, supporting positions, uvs and normals
		...
		// - Yes, vertices and indices are effectively the same since OBJs do not index entire vertices!  This means
		...
		//Calculate and update vertex tangents
